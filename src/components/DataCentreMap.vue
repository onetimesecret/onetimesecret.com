...
<template>
  <div class="data-center-map-container">
    <div class="flex flex-col">
      ...
      <!-- Map Area -->
      <div class="relative" :style="{ width: mapWidth, height: mapHeight }">
        <div class="absolute inset-0 border rounded-lg overflow-hidden bg-gray-100">
          <!-- World Map SVG -->
          <svg
          ...
          >
            ...
            <!-- Data Center Markers -->
            <g v-for="dataCenter in dataCenters" :key="dataCenter.id">
              <circle
      ...
      <div class="mt-4 p-4 border rounded-lg bg-white shadow-sm" v-if="selectedDataCenter">
        <h3 class="text-lg font-semibold">{{ selectedDataCenter.name }}</h3>
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-2">
          <div>
            <p class="text-sm text-gray-600">Location: <span class="font-medium text-gray-800">{{ selectedDataCenter.location }}</span></p>
            <p class="text-sm text-gray-600">Coordinates:
              <span class="font-medium text-gray-800">
                {{ selectedDataCenter.coordinates.lat.toFixed(4) }}, {{ selectedDataCenter.coordinates.lng.toFixed(4) }}
              </span>
            </p>
          </div>
          <div>
            <p class="text-sm text-gray-600">Status:
              <span class="inline-flex items-center">
                <span class="w-2 h-2 rounded-full mr-1.5" :class="getStatusColor(selectedDataCenter.status)"></span>
                <span class="font-medium text-gray-800">{{ selectedDataCenter.status.charAt(0).toUpperCase() + selectedDataCenter.status.slice(1) }}</span>
              </span>
            </p>
          </div>
        </div>
      ...
      <div v-else class="mt-4 p-4 border rounded-lg bg-white shadow-sm text-gray-500 italic">
