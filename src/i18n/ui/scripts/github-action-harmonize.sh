...
ls -la "${GITHUB_WORKSPACE}/src/i18n/ui/scripts/"
...
chmod +x "${GITHUB_WORKSPACE}/src/i18n/ui/scripts/harmonize-locale-files"
chmod +x "${GITHUB_WORKSPACE}/src/i18n/ui/scripts/harmonize-locale-file"

# Run the harmonize script with verbose output
"${GITHUB_WORKSPACE}/src/i18n/ui/scripts/harmonize-locale-files" -v $COPY_OPTION

# Store exit code
HARMONIZE_EXIT_CODE=$?

# Check if changes were made to any locale files
if git diff --name-only | grep -q "src/i18n/ui/"; then
